...
pub struct Note {
    ...
    ///
    /// ZSA notes ([ZIP 226]) always use `NoteVersion::V2`: they keep the ZIP 212
    /// rcm derivation, and their `0x03` plaintext lead byte is an encoding
    /// detail of the ZSA note plaintext format, not a [ZIP 2005] version marker.
    ///
...
impl Note {
    ...
    pub(crate) fn new_issue_note(
        ...
        Note {
            ...
            // ZSA issuance notes use the ZIP 212 rcm derivation (see `Note::version`).
...
mod tests {
    ...
    use crate::{
        ...
        test_vectors::keys_vanilla::TestVector,
    };
    use ff::PrimeField;
    use group::GroupEncoding;
    ...
    fn qr_rcm_differs_from_old_rcm() {
        let tv = &crate::test_vectors::keys_vanilla::test_vectors()[0];
    ...
    fn qr_rcm_verify_stored_vectors() {
        for (i, key_tv) in crate::test_vectors::keys_vanilla::test_vectors()
            .iter()
            .enumerate()
        {
