...
// Instance row 9 carries the circuit-flavor-specific flag: `enableZSA` for the OrchardZSA
// circuit, and `disableCrossAddress` for the post-NU6.3 vanilla circuit. These flags are
// mutually exclusive by construction (see [`Instance::from_parts`]), and instance columns
// are zero-padded, so for statements where the flag is zero this encoding is
// commitment-identical to the historical nine-row encoding.
...
///
/// The `circuit_version` field of the wrapped [`Witnesses`] selects which circuit to
/// build. Callers must choose it explicitly instead of relying on a default.
#[derive(Clone, Debug)]
...
impl Witnesses {
    /// Returns empty witnesses with all private values unknown.
    ///
    /// This is used for circuit shape-dependent operations, such as generating keys
    /// or rendering the circuit layout, where witness values are not required but the
    /// selected circuit version still determines the configured constraints.
...
///
/// Build with [`VerifyingKey::build`] for an explicit circuit version. The key
...
impl Instance {
    ...
    ///
    /// Also returns `None` if `flags` both enables ZSA and disables cross-address
    /// transfers: the two flags share instance row 9 (`enableZSA` for the OrchardZSA
    /// circuit, `disableCrossAddress` for the post-NU6.3 vanilla circuit), and no
    /// bundle version can represent that combination.
    ///
    /// See:
    ...
    pub fn from_parts(
        ...
        // Instance row 9 carries either `enableZSA` or `disableCrossAddress`; a single
        // instance cannot assert both, so reject flags that are ZSA-enabled and
        // cross-address-disabled at once (no bundle version produces such flags).
        let flags_representable = !flags.zsa_enabled() || flags.cross_address_enabled();
        (!rk.is_identity() && flags_representable).then_some(Instance {
            anchor,
            cv_net,
            nf_old,
            rk,
    ...
    /// With ZSA and the post-NU6.3 circuit, it now returns 10 elements, the last one carrying
    /// the circuit-flavor-specific flag (`enableZSA` or `disableCrossAddress`, which are
    /// mutually exclusive by construction). When the flag is zero, halo2_proofs's zero-padding
    /// of instance values makes old proofs (without this element) and new proofs behave
    /// identically.
    fn to_halo2_instance(&self) -> [[vesta::Scalar; 10]; 1] {
        ...
        // Rows `ENABLE_ZSA` and `DISABLE_CROSS_ADDRESS` are the same index; the two flags
        // are mutually exclusive by construction (`Instance::from_parts`), so at most one
        // of these writes is nonzero. Instance columns are zero-padded over the evaluation
        // domain, so for statements where the flag is false, this encoding is
        // commitment-identical to the historical nine-row encoding. Circuits that leave
        // this row unconstrained must never receive statements that set it (see
        // `Proof::create` and `Proof::verify`).
