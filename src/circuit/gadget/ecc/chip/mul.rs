use super::{add_complete, double, util, CellValue, EccConfig, EccPoint};
...
pub(super) fn assign_region<C: CurveAffine>(
    ...
    let k_bits = decompose_scalar::<C>(scalar);
    ...
    let acc = double::assign_region::<C>(&base, offset, region, config.clone())?;
    ...
    region.assign_fixed(
        ...
        k_0_row + offset,
        || Ok(C::Base::from_bytes(&scalar.to_bytes()).unwrap()),
    )?;
...
fn decompose_scalar<C: CurveAffine>(scalar: C::Scalar) -> Vec<bool> {
