...
impl OrchardZSADomain {
    ...
    pub fn for_nullifier(nullifier: Nullifier) -> Self {
        OrchardZSADomain { rho: nullifier }
    }
}
...
pub type OrchardNoteEncryption = zcash_note_encryption_zsa::NoteEncryption<OrchardZSADomain>;

impl<T> ShieldedOutput<OrchardZSADomain> for Action<T> {
...
where
    Action<T>: ShieldedOutput<OrchardZSADomain>,
{
...
impl ShieldedOutput<OrchardZSADomain> for CompactAction {
    fn ephemeral_key(&self) -> EphemeralKeyBytes {
...
mod tests {
    ...
    use super::{
        ...
        OrchardZSADomain, OrchardNoteEncryption,
    };
    use crate::{
        action::Action,
        keys::{
    ...
    proptest! {
        ...
        fn test_encoding_roundtrip(
            ...
            // Encode.
            let mut plaintext = OrchardZSADomain::note_plaintext_bytes(&note, memo);

            // Decode.
            let domain = OrchardZSADomain { rho: note.rho() };
    ...
    fn test_vectors() {
        ...
        for tv in test_vectors {
            ...
            let domain = OrchardZSADomain { rho };
