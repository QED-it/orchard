...
impl OutputInfo {
    ...
    ///
    /// [orcharddummynotes]: https://zips.z.cash/protocol/nu5.pdf#orcharddummynotes
    pub fn dummy(note_version: NoteVersion, rng: &mut impl RngCore, asset: AssetBase) -> Self {
    ...
    fn build<Pr: OrchardPrimitives>(
        ...
        // Every note plaintext version shares encryption behavior; `Note::version()`
        // selects the note plaintext lead byte, and the flavor `Pr` the encoding.
        ...
        let enc_ciphertext = if self.randomized_ciphertext {
            ...
            let mut enc_ciphertext = vec![0u8; Pr::ENC_CIPHERTEXT_SIZE];
            rng.fill_bytes(&mut enc_ciphertext);
            Pr::NoteCiphertextBytes::from_slice(&enc_ciphertext)
                .expect("buffer has size Pr::ENC_CIPHERTEXT_SIZE")
        } else {
...
impl ChangeInfo {
    ...
    pub fn new(
        ...
        Ok(Self {
            // Change outputs are wallet-native and always denominated in zatoshi; the
            // cross-address-disabled bundles that use them are vanilla-only.
...
/// Builds a bundle containing the given spent notes, outputs, wallet-controlled change
/// outputs, and burns, under the given [`BundleVersion`] (which selects the Action circuit
...
pub fn bundle<V: TryFrom<i64>, FL: OrchardFlavor>(
    ...
    build_bundle(
        ...
        |pre_actions, flags, value_balance, burn_vec, bundle_meta, rng| {
            finish_unauthorized_bundle::<V, FL, _>(
                pre_actions,
                flags,
                value_balance,
                burn_vec,
...
fn finish_unauthorized_bundle<V: TryFrom<i64>, FL: OrchardFlavor, R: RngCore>(
    ...
    let result_value_balance: V = V::try_from(zatoshi_value_balance)
    ...
    let (actions, witnesses): (Vec<_>, Vec<_>) = pre_actions
        .into_iter()
        .map(|a| a.build::<FL>(&mut rng, circuit_version))
        .unzip();
...
fn build_bundle<B, R: RngCore>(
    ...
    let (pre_actions, bundle_meta) = if !flags.cross_address_enabled() {
        ...
        //
        // This branch is vanilla-only by construction: the ZSA pool always has
        // cross-address transfers enabled (see `Flags::to_byte`), so all notes here use
        // the zatoshi asset.
        //
        ...
        // the minimum spends/outputs (e.g. a bundle that only burns a custom asset).
...
pub mod testing {
    ...
    use crate::{
        ...
        bundle::{testing::ArbBundleVersion, Authorized, Bundle},
        circuit::ProvingKey,
        flavor::OrchardFlavor,
    ...
    impl<R: RngCore + CryptoRng> ArbitraryBundleInputs<R> {
        ...
        fn into_bundle<V: TryFrom<i64> + Copy + Into<i64>, FL: OrchardFlavor + ArbBundleVersion>(
            mut self,
    ...
    impl<FL: OrchardFlavor + ArbBundleVersion> BuilderArb<FL> {
        prop_compose! {
...
mod tests {
    ...
    fn shielding_bundle<FL: OrchardFlavor>(bundle_version: BundleVersion, flags: Flags) {
        let pk = ProvingKey::build::<FL>(bundle_version.circuit_version());
        let mut rng = OsRng;
    ...
    fn shielding_bundle_vanilla() {
        ...
        shielding_bundle::<OrchardVanilla>(bundle_version, bundle_version.default_flags())
    }

    #[test]
    fn shielding_bundle_zsa() {
        let bundle_version = BundleVersion::zsa_v3();
        shielding_bundle::<OrchardZSA>(bundle_version, bundle_version.default_flags())
    }

    #[test]
    fn shielding_bundle_zsa_with_vanilla_flags() {
        shielding_bundle::<OrchardZSA>(BundleVersion::zsa_v3(), Flags::ENABLED)
    }
    ...
    fn cross_address_disabled_spend_output_randomization_depends_on_scope() {
        fn spend_paired_output_decrypts(spend_scope: Scope) -> bool {
            ...
            let domain = OrchardDomain::<OrchardVanilla>::for_pczt_action(spend_action);
