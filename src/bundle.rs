...
use crate::{
    ...
    primitives::{BundleDomain, OrchardPrimitives},
...
impl BundleVersion {
    ...
    /// The [`BundleVersion`] for the [`ValuePool::Zsa`] pool ([ZIP 226]).
    ///
    ...
    pub fn circuit_version(&self) -> OrchardCircuitVersion {
        match (self.value_pool, self.protocol_version) {
            // The OrchardZSA circuit is defined on the post-NU6.2 fixed circuit version;
            // it does not implement the post-NU6.3 cross-address restriction.
            (ValuePool::Zsa, _) => OrchardCircuitVersion::FixedPostNu6_2,
            (_, ProtocolVersion::InsecureV1) => OrchardCircuitVersion::InsecurePreNu6_2,
            (_, ProtocolVersion::V2) => OrchardCircuitVersion::FixedPostNu6_2,
            (_, ProtocolVersion::V3) => OrchardCircuitVersion::PostNu6_3,
        }
    }
    ...
    pub fn note_version(&self) -> NoteVersion {
        match self.value_pool {
            ...
            // ZSA notes keep the ZIP 212 rcm derivation (see `Note::version`); their 0x03
            // plaintext lead byte is part of the ZSA encoding, not a ZIP 2005 marker.
            ValuePool::Zsa => NoteVersion::V2,
        }
    }
    ...
    pub fn default_flags(&self) -> Flags {
        Flags::from_parts(
            ...
            matches!(self.value_pool, ValuePool::Zsa),
        )
    }
...
/// Bit 2 of the v6 flag byte. For the [`ValuePool::Ironwood`] pool it encodes
/// `enableCrossAddress` (ZIP 229); for the [`ValuePool::Zsa`] pool it encodes
/// `enableZSA` (ZIP 226). For the [`ValuePool::Orchard`] pool it is reserved
/// and MUST be 0.
...
impl Flags {
    /// Construct a set of flags from its constituent parts, including the cross-address
    /// and ZSA bits.
    ///
    ...
    /// expanded receivers. ZSA functionality is disabled.
    ///
    ...
    /// spent note is constrained to be a dummy, while outputs may be real. ZSA
    ...
    /// created note is constrained to be a dummy, while spends may be real. ZSA
    /// functionality is disabled.
    ///
    ...
    pub fn to_byte(&self, bundle_version: BundleVersion) -> Option<u8> {
        ...
        // The ZSA flag is only representable in the ZSA pool, where it occupies bit 2.
        if self.zsa_enabled {
            if !matches!(bundle_version.value_pool, ValuePool::Zsa) {
                return None;
            }
        ...
        match (bundle_version.value_pool, bundle_version.protocol_version) {
            ...
            // No cross-address restriction is defined for the ZSA pool (ZIP 226), so a
            // disabled flag set cannot be represented; bit 2 carries `enableZSA` instead.
            (ValuePool::Zsa, ProtocolVersion::V3) => {
                if !self.cross_address_enabled {
                    return None;
                }
            }
            // The ZSA pool is not defined prior to ProtocolVersion::V3.
    ...
    pub fn from_byte(value: u8, bundle_version: BundleVersion) -> Option<Self> {
        ...
        // Bit 2 can only be 1 for an Ironwood or ZSA bundle. For Orchard it MUST be 0,
        // independent of the tx version:
        //
        ...
        // We have already validated bit2 against the pool type. For the ZSA pool, bit 2
        // encodes `enableZSA` (ZIP 226) and no cross-address restriction is defined.
        let (cross_address_enabled, zsa_enabled) = match bundle_version.value_pool {
            ValuePool::Zsa => (true, bit2),
            ValuePool::Orchard | ValuePool::Ironwood => (
                match bundle_version.protocol_version {
...
impl<V, Pr: OrchardPrimitives> Bundle<Authorized, V, Pr> {
    ...
    /// for a given [`OrchardSighashKind`] (used by the ZIP 246 v6 digests).
    ///
    /// # Errors
    ///
    /// Returns [`CommitmentError::InvalidTransactionVersion`] if `tx_version` is not valid for the
...
pub mod testing {
    ...
    use crate::{
        ...
        Anchor, NoteVersion, Proof, ValuePool,
    };

    use super::{Action, Authorized, Bundle, BundleVersion, Flags};
    ...
    /// Selects the [`BundleVersion`]s under which a flavor's bundles can be generated.
    pub trait ArbBundleVersion {
        /// Create an arbitrary [`BundleVersion`] valid for this flavor.
        fn arb_bundle_version() -> BoxedStrategy<BundleVersion>;

        /// The default [`BundleVersion`] used when tests need a single fixed version
        /// for this flavor.
        fn default_bundle_version() -> BundleVersion;
    }

    impl ArbBundleVersion for crate::flavor::OrchardVanilla {
        fn arb_bundle_version() -> BoxedStrategy<BundleVersion> {
            arb_bundle_version().boxed()
        }
    ...
    impl ArbBundleVersion for crate::flavor::OrchardZSA {
        fn arb_bundle_version() -> BoxedStrategy<BundleVersion> {
            Just(BundleVersion::zsa_v3()).boxed()
        }

        fn default_bundle_version() -> BundleVersion {
            BundleVersion::zsa_v3()
        }
    }

    /// Returns `flags` with its `cross_address_enabled` and `zsa_enabled` bits forced to
    /// values representable under `bundle_version`, leaving the spend/output bits untouched.
    ///
    ...
    fn flags_for_version(bundle_version: BundleVersion, flags: Flags) -> Flags {
        Flags::from_parts(
            ...
            matches!(bundle_version.value_pool(), ValuePool::Zsa) && flags.zsa_enabled(),
        )
    }
    ...
    impl<Pr: OrchardPrimitives + ArbBundleVersion + Default> BundleArb<Pr> {
        ...
        prop_compose! {
            ...
            pub fn arb_flags()(spends_enabled in prop::bool::ANY, outputs_enabled in prop::bool::ANY, zsa_enabled in prop::bool::ANY) -> Flags {
                Flags::from_parts(spends_enabled, outputs_enabled, true, zsa_enabled)
            }
        }
        ...
        prop_compose! {
            ...
            (
                bundle_version in Pr::arb_bundle_version(),
...
pub(crate) mod tests {
    ...
    pub(crate) fn with_cross_address_disabled(
        ...
        flags.zsa_enabled = false;

        // The sampled bundle's version may predate NU6.3, where the cross-address-disabled
        // flag set is unrepresentable; rebuild under a version that mandates it.
    ...
    pub(crate) fn sample_authorized_bundle(
        ...
        BundleArb::<OrchardVanilla>::arb_bundle(n_actions)
            .new_tree(&mut runner)
    ...
    fn try_from_parts_enforces_canonical_proof_size_for<Pr: OrchardPrimitives>(
        ...
        // A version that enforces canonical proof sizes and can represent the
        // sampled bundle's flags.
        bundle_version: BundleVersion,
    ...
    proptest! {
        ...
        fn try_from_parts_enforces_canonical_proof_size_vanilla(
            ...
            try_from_parts_enforces_canonical_proof_size_for::<OrchardVanilla>(
                bundle,
                BundleVersion::ironwood_v3(),
            )?;
        }
        ...
        fn try_from_parts_enforces_canonical_proof_size_zsa(
            ...
            try_from_parts_enforces_canonical_proof_size_for::<OrchardZSA>(
                bundle,
                BundleVersion::zsa_v3(),
            )?;
        }
        ...
        #[test]
        fn arb_flags_ironwood_post_nu6_3_round_trips(
            flags in BundleArb::<OrchardVanilla>::arb_flags_ironwood_post_nu6_3()
        ) {
            let encoded = flags
        ...
        #[test]
        fn orchard_nu6_3_rejects_cross_address_enabled(
            flags in BundleArb::<OrchardVanilla>::arb_flags()
        ) {
            ...
            // The ZSA flag is likewise only representable in the ZSA pool.
            disabled.zsa_enabled = false;
        ...
        #[test]
        fn commitment_hashes_the_wire_flag_byte(
            bundle in BundleArb::<OrchardVanilla>::arb_bundle(3)
        ) {
            let actions = bundle.actions().clone();
            let anchor = *bundle.anchor();
        ...
        #[test]
        fn ironwood_rejects_v5_commitment_version(
            bundle in BundleArb::<OrchardVanilla>::arb_bundle(3)
        ) {
            ...
            prop_assert!(matches!(
                ironwood.authorizing_commitment(TxVersion::V5, |_| vec![]),
                Err(CommitmentError::InvalidTransactionVersion)
            ));
        }
        ...
        #[test]
        fn anchor_placement_follows_tx_version(
            bundle in BundleArb::<OrchardVanilla>::arb_bundle(3)
        ) {
            ...
            for (bundle_version, tx, anchor_in_txid_digest) in [
                ...
                let auth_a = a.authorizing_commitment(tx, |_| vec![]).unwrap().0;
                let auth_b = b.authorizing_commitment(tx, |_| vec![]).unwrap().0;
    ...
    fn verify_proof_rejects_cross_address_disabled_for_unsupported_keys() {
        ...
        for circuit_version in [
            ...
            let vk = crate::circuit::VerifyingKey::build::<crate::flavor::OrchardVanilla>(
                circuit_version,
            );
