...
mod tests {
    ...
    fn shielding_bundle<FL: OrchardFlavor>(
        ...
        assert_eq!(orchard_digest.as_bytes(), expected_orchard_digest);
    ...
    fn shielded_bundle<FL: OrchardFlavor>(bundle_type: BundleType, expected_orchard_digest: &[u8]) {
        let pk = ProvingKey::build::<FL>();
        let mut rng = StdRng::seed_from_u64(1u64);
    ...
    fn shielded_bundle_orchard_vanilla() {
        shielded_bundle::<OrchardVanilla>(
            BundleType::DEFAULT_VANILLA,
            // `orchard_digest` taken from the `zcash/orchard` repository at commit `4fa6d3b`.
            // This ensures backward compatibility.
            &[
