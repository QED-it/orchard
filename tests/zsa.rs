...
fn build_and_sign_bundle(
    ...
    let unauthorized = builder
        ...
        .unwrap()
        .0;
    let sighash = unauthorized.commitment(TxVersion::V6).unwrap().into();
...
fn build_and_verify_bundle(
    ...
    let shielded_bundle: Bundle<_, i64, OrchardZSA> = {
        let mut builder = Builder::new(
            BundleType::DEFAULT,
            BundleVersion::zsa_v3(),
            BundleVersion::zsa_v3().default_flags(),
            anchor,
        )
        .unwrap();
    ...
    verify_bundle(&shielded_bundle, keys.vk, TxVersion::V6, true);
...
fn zsa_issue_and_transfer() {
    ...
    let pk = ProvingKey::build::<OrchardZSA>(BundleVersion::zsa_v3().circuit_version());
    let vk = VerifyingKey::build::<OrchardZSA>(BundleVersion::zsa_v3().circuit_version());
